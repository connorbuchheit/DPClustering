...
def dp_dbscan_core_point_count(data, eps, min_samples, dp_eps):
    """
    ...
    def noisy_count(count, dp_eps, total_points):
        noisy_count = count + np.random.laplace(0, 1/dp_eps) # Global sensitivty of count is 1 
        # But not sure if this is the right way to do it.
        # TODO — figure out the proper magnitude for the SD of the noise.
        noisy_count = int(round(noisy_count)) 
        return max(0, min(noisy_count, total_points)) # Ensure count is non-negative and doesn't exceed total points
    ...
    for point_idx in range(len(data)):
        ...
        noisy_neighbors = noisy_count(len(neighbors), dp_eps, len(data)) # Noised count of neighbors

        if noisy_neighbors < min_samples:
